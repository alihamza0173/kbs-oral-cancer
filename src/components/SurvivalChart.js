...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-white border border-clinical-200 shadow-lg rounded-lg px-3 py-2">
      <p className="text-xs font-semibold text-slate-700">Month {d.month}</p>
      <p className="text-sm font-bold text-clinical-700">{(d.survival * 100).toFixed(1)}% survival</p>
    </div>
  );
}
    ...
    <div className="bg-white rounded-xl shadow-sm border border-clinical-200 p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Fit from clinical dataset · patient-adjusted</p>
        </div>
      </div>

      <div className="flex gap-6 mb-4">
        <div className="flex-1 bg-clinical-50 rounded-lg p-3 text-center border border-clinical-100">
          <p className="text-2xl font-bold text-clinical-700">{(survival.threeYear * 100).toFixed(0)}%</p>
          <p className="text-xs text-clinical-600 font-medium">3-Year Survival</p>
        </div>
        <div className="flex-1 bg-clinical-50 rounded-lg p-3 text-center border border-clinical-100">
          <p className="text-2xl font-bold text-clinical-700">{(survival.fiveYear * 100).toFixed(0)}%</p>
          <p className="text-xs text-clinical-600 font-medium">5-Year Survival</p>
        </div>
      </div>
      ...
      <ResponsiveContainer width="100%" height={240}>
        <AreaChart data={data} margin={{ top: 5, right: 10, bottom: 5, left: -10 }}>
          ...
          <ReferenceLine x={36} stroke="#0d9488" strokeDasharray="4 4" strokeOpacity={0.5} />
          <ReferenceLine x={60} stroke="#0f766e" strokeDasharray="4 4" strokeOpacity={0.5} />
          <Area
            type="stepAfter"
            dataKey="survivalPct"
            stroke="#0d9488"
