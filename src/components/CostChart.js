...
function CustomTooltip({ active, payload }) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-lg rounded-lg px-3 py-2">
      <p className="text-xs font-semibold text-slate-700">{d.phase}</p>
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-800">Cost Analysis</h2>
            <p className="text-xs text-slate-500">Phase-based economic evaluation</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-2 mt-4">
        {costData.map((item, i) => (
          <div key={i} className="text-center p-2 rounded-lg bg-slate-50">
            ...
            <p className="text-xs font-semibold text-slate-700">${item.cost.toLocaleString()}</p>
            <p className="text-[10px] text-slate-400">{item.phase}</p>
          </div>
        ))}
      </div>
    </div>
